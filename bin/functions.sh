...
ChromeInst() {
    ...
    if [ $? -eq 0 ] ; then
        echo "[OK] Chrome 다운로드가 완료되었습니다."
    else
        echo "[FAIL] Chrome 다운로드 실패"
        exit 2
    fi
    CHROME_PKG=$(basename $URL)
    yum  -qy install  $PKG_DOWN_DIR/$CHROME_PKG > /dev/null 2>&1
    if [ $? -eq 0 ] ; then
        echo "[OK] Chrome 패키지 설치가 완료되었습니다."
    else
        echo "[FAIL] Chrome 패키지 설치가 설치되지 않았습니다."
    fi
}
...
SshConf() {
    ...
    sed -i 's/#PermitRootLogin no/PermitRootLogin/' $SSHD_CONF
